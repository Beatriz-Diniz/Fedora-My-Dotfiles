...
get_hashmap() {
    ...
    for wallSource in "$@"; do
        ...
        hashMap=$(find "${wallSource}" -type f \( -iname "*.gif" -o -iname "*.mp4" -o -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" \) -exec "${hashMech}" {} + | sort -k2)
        if [ -z "${hashMap}" ]; then
...
generate_thumbnail() {
    local videoFile="$1"
    local outputThumbnail="$2"
    echo "Generating thumbnail for video: ${videoFile}"
    magick convert -thumbnail 512x288 -background black -gravity center "${videoFile}[0]" "${outputThumbnail}"
}

# Example usage of generate_thumbnail
# generate_thumbnail "/home/beatrizdiniz/.config/hyde/themes/Catppuccin Mocha/wallpapers/mylivewallpapers-com-Halloween-Cat-4K.mp4" "/home/beatrizdiniz/.cache/hyde/thumbs/your_thumbnail.png"
